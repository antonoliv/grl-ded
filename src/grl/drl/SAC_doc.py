...
def td_targets(rewards, discounts, next_values):
    ...
    :param discounts:       discount factors
    :param next_values:     next values
    :return: TD targets
    """
...
def compute_Q_targets(next_Q_values,
    ...
    """
    TODO: Add docstring
    Static Function to compute the Q targets
    :param next_Q_values:   Q values for next states
    :param next_log_pis:    log probabilities of next actions
    :param rewards:         next rewards
    :param terminals:       terminal values
    :param discount:        discount factor (gamma)
    :param entropy_scale:   entropy scale (alpha)
    ...
    # Cast terminals to next values data type
    terminals = tf.cast(terminals, next_values.dtype)

    # Compute Q targets using TD targets
...
class SAC(Algorithm):
    ...
    def _compute_Q_targets(self, batch):
        """
        Compute Q targets from replay buffer batch data
        :param batch: replay buffer batch
        :return:
        """
        # Get next observations, current rewards and terminal values from batch
        next_observations = batch['next_observations']
        rewards = batch['rewards']
        terminals = batch['terminals']

        # Convert alpha, reward scale and gamma to tensors
        ...
        # Get next actions and log probabilities from policy
        next_actions, next_log_pis = self._policy.actions_and_log_probs(
            next_observations)

        # Get Q values for each Q target function
        next_Qs_values = tuple(
            Q.values(next_observations, next_actions) for Q in self._Q_targets)

        # Get min Q values from Q target functions
    ...
    def _update_critic(self, batch):
        ...
        # Get observations, rewards and actions from replay buffer
