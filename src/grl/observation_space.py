...
class GraphObservationSpace(spaces.Dict):
    def __init__(self, init_space):
        ...
        time = spaces.Box(
                np.full(shape=(1,), fill_value=np.float32(0), dtype=np.float32),
                np.full(shape=(1,), fill_value=np.inf, dtype=np.float32),
                (1,),
                np.float32,
            )
    ...
    def to_gym(self, observation):
        ...
        return {
            ...
            "time": np.array([observation.delta_time]),
    ...
    def _get_x_space(self, init_space):
        ...
        x_space = spaces.Box(
            np.array([-np.inf, -np.inf, -extra_losses, -extra_losses, -extra_losses] * self.n_nodes, dtype=np.float32).reshape(x_shape),
            high,
            x_shape,
            np.float32
        )
