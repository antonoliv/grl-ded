...
class ExperimentRunner(tune.Trainable):
    def setup(self, params):
        ...
        if params['run_params'].get('run_eagerly', False):
    ...
    def _build(self):
        ...
        params['Q_params']['config'].update({
            'input_shapes': (
                training_environment.observation_shape,
                training_environment.action_shape),
        })
        Qs = self.Qs = tree.flatten(value_functions.get(params['Q_params']))

        params['policy_params']['config'].update({
        ...
        policy = self.policy = policies.get(params['policy_params'])

        params['replay_pool_params']['config'].update({
            'environment': training_environment,
        })
        replay_pool = self.replay_pool = replay_pools.get(
            params['replay_pool_params'])

        params['sampler_params']['config'].update({
            'environment': training_environment,
            'policy': policy,
            'pool': replay_pool,
        })
        sampler = self.sampler = samplers.get(params['sampler_params'])

        params['algorithm_params']['config'].update({
        ...
        self.algorithm = algorithms.get(params['algorithm_params'])
    ...
    def _save_replay_pool(self, checkpoint_dir):
        if not self._params['run_params'].get(
