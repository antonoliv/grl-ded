...
class DynamicEconomicReward(BaseReward):
    ...
    def __call__(self, action, env, has_error, is_done, is_illegal, is_ambiguous):
        if has_error or is_illegal or is_ambiguous:
            ...
            cost = dt_float(
                (env.get_obs(_do_copy=False).gen_p * env.gen_cost_per_MW).sum() * env.delta_time_seconds / 3600.0)
            ...
            for i in range(0, env.n_gen):
                if env.gen_renewable[i]:
                    res += dt_float(
                        env.get_obs(_do_copy=False).gen_p_before_curtail[i] - env.get_obs(_do_copy=False).gen_p[i])
