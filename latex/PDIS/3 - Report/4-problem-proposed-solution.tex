...
\begin{displayquote}
	 \textit{The reviewed \ac{RL} approaches are not prepared to consider graph topology features in the decision making process in the context of graph-based environments}
\end{displayquote}
...
By conducting a thorough literature review of the relevant studies in the field we observed that current \ac{RL} algorithms are not prepared to handle such complexity by considering environment graph topology features in the decision-making process, which deeply affects the performance of decision-making systems inserted in graph-based domains. Additionally, they lack an efficient method for generalizing the learned policies in dynamic topologies that suffer changes over time.
 \par
...
More and more, this issue attracts the curiosity of academics. With the recent advancements of \acp{GNN} the popularity around \ac{GRL} has risen because of their excellent efficiency in creating optimal graph representations and other graph machine learning problems.
...
\begin{figure}
	...
	\caption{Grid2Op l2rpn\_idf\_2023 118-bus test case \cite{rtefranceGrid2OpDocumentation}}
